...
from sina_com.task.save_to_xlsx import TopicXLSX
    ...
    def parse_detail_topic_content(self):
        ...
        for topic in self.read_local_topics():
            ...
            if is_crawled:
                ...
                self.xlsx = TopicXLSX(topic.strip().replace("#", "").replace(":", "").replace("[","").replace("]","") + ".xlsx")
                ...
                while self.next_page_pool:
                    ...
                    try:
                        ...
                        # sleep(10)
                        log.error("话题【{}】处理失败,开始处理下一话题!!!".format(topic))
                        self.xlsx.close()
                        continue
                    ...
                    if next_page:
                        log.info("当前下载： {}".format(next_page[0]))
                        self.next_page_pool.append("https:" + next_page[0])
                    else:
                        self.next_page_pool.clear()

                    self.get_topic_details(selector)

                print("当前话题【{}】写入完毕".format(topic))
                self.xlsx.close()

    def get_topic_details(self, selector):
        ...
        nickname.extend(list(
            selector.xpath('//div[@action-type="feed_list_item"]//div[@class="card-feed"]//div[@class="content"]/div[@class="info"]/div[2]/a/@nick-name')))
        tmp_user_auth = selector.xpath('//div[@action-type="feed_list_item"]//div[@class="card-feed"]//div[@class="content"]/div[@class="info"]/div[2]')
        ...
        content_lst = selector.xpath('//div[@action-type="feed_list_item"]//div[@class="card"]//div[@class="content"]/p[@node-type="feed_list_content"]')
        for i in content_lst:
            content.append(i.xpath('string(.)').strip())

        tmp_create_time = selector.xpath('//div[@action-type="feed_list_item"]//div[@class="card-feed"]//div[@class="content"]')
        ...
        tmp_device = selector.xpath('//div[@action-type="feed_list_item"]//div[@class="card-feed"]//div[@class="content"]')
        for i in tmp_device:
        ...
        tmp_collections = selector.xpath('//div[@action-type="feed_list_item"]//div[@class="card"]/div[@class="card-act"]/ul')
        ...
        user_weibo_href.extend(list(selector.xpath(
            '//div[@action-type="feed_list_item"]//div[@class="card"]//div[@class="content"]/div[@class="info"]/div[2]/a[@class="name"]/@href')))
...
if __name__ == "__main__":
    ...
    crawl.parse_detail_topic_content()
    print("over!")